  ...
  try {
    ...
    if (history.length > 0) {
      history.forEach((entry) => {
        ...
        div.innerHTML = `
          <p><strong>Quotation No:</strong> NSC/${entry.quotationNo} | <strong>Date:</strong> ${entry.date?.split("T")[0] || ""}</p>
          <button onclick="editQuotation('${entry.id}')">Edit Quotation</button>
        `;
